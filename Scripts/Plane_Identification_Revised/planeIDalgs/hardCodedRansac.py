...
def silhouette_score(planes, pointsPlanes):
    pointsPlanesCopy = pointsPlanes.copy()
    if(len(planes) == 1):
        return 0
    else:
        for i in range(len(planes)):
            plane = planes[i]
            A, B, C, D = plane[0], plane[1], plane[2], plane[3]
            x, y, z = pointsPlanesCopy[i].x, pointsPlanesCopy[i].y, pointsPlanesCopy[i].z 
            pointsPlanesCopy[i]["planeDistance"] = abs(A*x+ B*y + C*z + D) / math.sqrt(A**2 + B**2 + C**2)
            pointsPlanesCopy[i]["outerDistance"] = np.inf
            for j in range(len(planes)):
                if j != i:
                    plane = planes[j]
                    A, B, C, D = plane[0], plane[1], plane[2], plane[3]
                    pointsPlanesCopy[i]["newDistances"] = abs(A * pointsPlanesCopy[i]["x"] + B * pointsPlanesCopy[i]["y"] + C * pointsPlanesCopy[i]["z"] + D) / math.sqrt(A**2 + B**2 + C**2)
                    pointsPlanesCopy[i]["outerDistance"] = pointsPlanesCopy[i][["outerDistance", "newDistances"]].min(axis=1)

            pointsPlanesCopy[i]["SilhouetteScore"] = (pointsPlanesCopy[i]["outerDistance"] - pointsPlanesCopy[i]["planeDistance"]) / pointsPlanesCopy[i][["outerDistance", "planeDistance"]].max(axis=1)

        return sum(pointsPlanesCopy[i]["SilhouetteScore"].sum() for i in range(len(pointsPlanesCopy))) / sum(len(pointsPlanesCopy[i]) for i in range(len(pointsPlanesCopy)))

def get_score(planes, pointsPlanes):
    pointsPlanesCopy = pointsPlanes.copy()
    for i in range(len(planes)):
        plane = planes[i]
        A, B, C, D = plane[0], plane[1], plane[2], plane[3]
        x, y, z = pointsPlanesCopy[i].x, pointsPlanesCopy[i].y, pointsPlanesCopy[i].z 
        pointsPlanesCopy[i]["planeDistance"] = abs(A*x+ B*y + C*z + D) / math.sqrt(A**2 + B**2 + C**2)
    return -sum(pointsPlanesCopy[i]["planeDistance"].sum() for i in range(len(pointsPlanesCopy))) 
    ...
    while(True):
        ...
        for i in range(n):
            ...
            if len(remaining_points) > 3:
                ...
                planes, planePoints, remaining_points = 
...
def ransacHeightSplit(lasDF, stoppingPercentage = 0.2):
    planes = []
    pointsPlanes = []
